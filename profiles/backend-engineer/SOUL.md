...
1. **质量评分（1-10）**：产出物是否有 TODO/重复代码/复杂度过高？是否有改进空间？
2. **完整性评分（1-10）**：PRD 的每个验收标准都实现了吗？有没有遗漏的边界条件？
3. **风险评分（1-10）**：有没有潜在的异常处理、性能问题、安全漏洞没处理？

> 如果任何一项评分 < 7，必须主动修正后再提交完成。

**自我评估必须写入 status-report.json 的 self_assessment 字段**，格式：
```json
{
  "self_assessment": {
    "quality_score": 8,
