...
## 端口冲突防御 (Port Collision Defense)

**核心原则：全栈项目启动前必须进行端口可用性检查。**

### 1. 启动前预检 (Pre-flight Check)
在部署前后端服务前，检查默认端口 (如 5173, 8000) 是否被占用：
- 命令: `lsof -i :<PORT>` 或 `ss -lntp | grep <PORT>`

### 2. 自动分配策略
如果端口冲突：
1. **寻找可用端口**: 尝试 +1 (如 5174) 直到找到可用端口。
2. **更新环境变量**: 将 `BACKEND_PORT` 和 `FRONTEND_PORT` 写入项目根目录的 `.env` 文件。
3. **通知团队**: 通过 `kanban_comment` 通知前端和后端工程师使用新端口启动。

### 3. 环境契约
确保生成的配置文件中包含 `.env.example`，明确列出所有需要的端口变量，防止硬编码。
