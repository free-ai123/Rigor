...
### SOUL.md — Role-Specific Profile Personality

Each profile has a `SOUL.md` file that injects role-specific instructions into the system prompt alongside the base `KANBAN_GUIDANCE` block. A well-structured SOUL.md has these sections:
...
## 工作流
开始任务前先检查 $HERMES_KANBAN_WORKSPACE/shared/ 是否有可复用知识。
1. 接收任务: kanban_show
2. 环境检查: 确认 workspace 路径
3. 执行
4. 结构化交付: kanban_complete with required metadata JSON
5. 审查门禁: kanban_block(reason="review-required: ...") for code changes
...
## 结构化通信协议
- JSON 格式的 handoff 模板 (type, from, to, content)

## 禁止行为
- 明确列出的禁止操作

## 知识沉淀
- 写入 shared/decisions/, shared/patterns/, shared/gotchas/

## 沟通风格
- 语言、语气
```

Key principles: every role must have explicit "what NOT to do" constraints, a structured `kanban_complete` metadata JSON template, and a `kanban_block` review-required clause for code changes. The structured communication protocol (JSON with type/from/to/content) enables cross-role handoffs via `kanban_comment`.
